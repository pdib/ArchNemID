...
ITEM_BOX_INNER_BORDER_WIDTH = 2
ITEM_BOX_COLOR = win32api.RGB(255, 0, 0)
TEXT_HEIGHT = 20
...
def display_overlay(target_window_name: str, recipes: dict, catalog: dict, recipe_idx: int, grid_x_offset: int, grid_y_offset: int, grid_width: int, grid_height: int, show_overlay: bool):
    try:
        if show_overlay:
            text, boxes = prepare_items_to_draw(
                grid_x_offset,
                grid_y_offset,
                grid_width,
                grid_height,
                ...
                catalog,
                recipe_idx)
            show_overlay_window(target_window_name, boxes, text)
        else:
            hide_overlay_window()
...
def show_overlay_window(target_window_name: str, boxes: list, text: tuple):
...
def update_overlay_window(hwnd, left, top, width, height, boxes, text):
...
def prepare_items_to_draw(offset_left: int, offset_top: int, width: int, height: int, recipes: dict, catalog: dict, recipe_idx: int):
    ...
    boxes += [(offset_left, offset_top, offset_left +
    ...
    boxes += [(
        offset_left + j * item_box_width,
        offset_top + i * item_box_height,
        offset_left + (j + 1) * item_box_width,
        offset_top + (i + 1) * item_box_height
    ) for (i, j) in organ_coords]
